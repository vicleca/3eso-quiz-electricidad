...
export async function loadCSV() {
  ...
  return new Promise((resolve, reject) => {
    Papa.parse(text, {
      ...
      skipEmptyLines: true,
      complete: (results) => {
        // 1. Carga TODAS las preguntas del CSV
        let allQuestions = buildQuestions(results.data);

        // 2. Mira la configuración para ver cuántas usar
        const numPreguntas = state.config.preguntasPorExamen || 0; // 0 significa "usar todas"

        // 3. Selecciona el subconjunto de preguntas
        if (numPreguntas > 0 && numPreguntas < allQuestions.length) {
          // Si queremos un subconjunto (ej: 20 de 50)
          // Baraja aleatoriamente el banco de preguntas completo
          shuffle(allQuestions);
          // Coge solo las primeras 'numPreguntas'
          state.preguntas = allQuestions.slice(0, numPreguntas);
        } else {
          // Usa todas las preguntas (si numPreguntas es 0 o > que el total)
          state.preguntas = allQuestions;
        }

        // 4. Continúa la inicialización normal con el set de preguntas seleccionado
        ...
        shuffle(state.orden); // Baraja el *orden* en que se preguntarán
        preCountTags(); // Pre-cuenta las etiquetas solo de las preguntas seleccionadas
        state.idx = 0;
        state.score = 0;
        setupNavigator();
        showQuestion();
...
function finalizeQuiz() {
  ...
  $("#r-date").textContent = dateStr;


  // Notas globales con barras (Rejilla original)
  ...
  // Función para la lista CONDENSADA (4-columnas)
  const fillListCondensed = (ul, items) => {
    ul.innerHTML = "";
    if (items.length === 0) {
    ...
    items.forEach(item => {
      ...
      li.innerHTML = `
        <span>${item.tag}</span>
        <span class="note ${getScoreClass(item.note)}">${format2(item.note)}/10</span>
      `;
      ul.appendChild(li);
    });
  };

  // Función para la lista DETALLADA (original)
  const fillListDetailed = (ul, items) => {
    ul.innerHTML = "";
    items.forEach(item => {
      const li = document.createElement("li");
      li.innerHTML = `
  ...
  // Rellenar ambas rejillas
  fillListCondensed($("#r-saberes-list-condensed"), state.detalle.sabList);
  fillListCondensed($("#r-comp-list-condensed"), state.detalle.compList);
  fillListCondensed($("#r-crit-list-condensed"), state.detalle.critList);
  fillListCondensed($("#r-desc-list-condensed"), state.detalle.descList);

  fillListDetailed($("#r-saberes-list"), state.detalle.sabList);
  fillListDetailed($("#r-comp-list"), state.detalle.compList);
  fillListDetailed($("#r-crit-list"), state.detalle.critList);
  fillListDetailed($("#r-desc-list"), state.detalle.descList);
